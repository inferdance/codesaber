...
export class SessionLog {
  ...
  append(type: string, payload: unknown, sync = false): number {
    const envelope: SessionEventEnvelope = {
      ts: Date.now(),
      seq: this.seq++,
      session_id: path.basename(this.path, ".jsonl"),
      type,
      payload,
    };
...
export function recoverSession(filePath: string): Recovered {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    try {
      ...
      if (!isLast) throw new Error(`corrupt at line ${i + 1}: ${e}`);
      break; // torn tail
    }
  }
  ...
  const unfinishedToolCalls = events
    .filter((e) => e.type === "tool_call" && !finished.has(e.payload.call_id))
    .map((e) => e.seq);
  return { events, unfinishedToolCalls };
}
